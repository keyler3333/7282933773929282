        ...
        func attemptRequest() {
            guard let apiKey = getNextAvailableKey() else {
                completion(.failure(NSError(
                    ...
                    code: -1,
                    userInfo: [NSLocalizedDescriptionKey: "No API keys configured. Please add keys in Settings."]
                )))
                return
            }

            guard let url = URL(string: "https://api.groq.com/openai/v1/chat/completions") else {
                completion(.failure(NSError(domain: "GroqAPIClient", code: -4, userInfo: [NSLocalizedDescriptionKey: "Invalid URL"])))
                return
            }
                ...
                if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 429 {
                    ...
                    if retryCount < self.maxRetries * self.apiKeys.count {
                        ...
                        completion(.failure(NSError(
                            domain: "GroqAPIClient",
                            code: 429,
                            userInfo: [NSLocalizedDescriptionKey: "Rate limited on all keys. Please wait or add more keys."]
                        )))
                    }
                    return
                }
